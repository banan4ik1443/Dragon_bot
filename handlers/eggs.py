...
from image_utils import render_card, get_dragon_image
...
async def cb_egg_open(callback: CallbackQuery) -> None:
    ...
    species_name = gd.species_display_name(species_key)
    ...
    image_path = get_dragon_image(species_key, "baby")
    await render_card(callback, reveal_text, kb.as_markup(), image_path)
