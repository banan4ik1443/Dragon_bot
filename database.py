...
async def migrate_db() -> None:
    """Добавляет новые поля в существующую БД если их нет."""
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            await db.execute("ALTER TABLE dragons ADD COLUMN growth_stage TEXT NOT NULL DEFAULT 'baby'")
        except Exception:
            pass
        try:
            await db.execute("ALTER TABLE dragons ADD COLUMN born_at TEXT")
            await db.execute("UPDATE dragons SET born_at = created_at WHERE born_at IS NULL")
        except Exception:
            pass
        await db.commit()
...
async def create_dragon(owner_id: int, species_key: str, rarity: str, name: str, ability_key: str) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            """INSERT INTO dragons
               (owner_id, species_key, name, rarity, fatigue_updated_at, ability_key, growth_stage, born_at, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (owner_id, species_key, name, rarity, _now(), ability_key, "baby", _now(), _now()),
        )
        await db.commit()
        return cur.lastrowid
