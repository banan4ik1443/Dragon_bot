...
# СТАТЫ И ПРОКАЧКА
# ──────────────────────────────────────────────────────────────────────────
def current_growth_stage(dragon: dict) -> str:
    """Возвращает текущую стадию роста дракона: baby / teen / adult."""
    stage = dragon.get("growth_stage", "baby")
    if stage == "adult":
        return "adult"
    # Проверяем не пора ли перейти на следующую стадию
    stage_data = gd.GROWTH_STAGES.get(stage)
    if not stage_data or stage_data["hatch_hours"] == 0:
        return stage
    ...
    # Суммируем часы по всем предыдущим стадиям
    total_hours = 0
    for s in order[:idx + 1]:
        total_hours += gd.GROWTH_STAGES[s]["hatch_hours"]
    if hours_passed >= total_hours and idx + 1 < len(order):
        return order[idx + 1]
    return stage


def growth_stage_info(dragon: dict) -> dict:
    stage = current_growth_stage(dragon)
    stage_data = gd.GROWTH_STAGES[stage]
    born_at = dt.datetime.fromisoformat(dragon.get("born_at") or dragon["created_at"])
    hours_passed = (dt.datetime.utcnow() - born_at).total_seconds() / 3600
    order = gd.GROWTH_ORDER
    if stage == "adult":
        hours_left = 0
        ...
        hours_left = max(0.0, round(total_needed - hours_passed, 1))
        next_stage = order[idx + 1] if idx + 1 < len(order) else None
    return {"stage": stage, "name": stage_data["name"], "emoji": stage_data["emoji"],
            "stat_mult": stage_data["stat_mult"], "hours_left": hours_left, "next_stage": next_stage}
...
def effective_stats(dragon: dict) -> dict:
    ...
    element_bonus = gd.ELEMENT_POWER_BONUS.get(element, 1.0)
    level = dragon["level"]
    fatigue = current_fatigue(dragon["fatigue"], dragon["fatigue_updated_at"], dragon["is_resting"])
    fmult = fatigue_stat_multiplier(fatigue)
    growth = growth_stage_info(dragon)
    growth_mult = growth["stat_mult"]
    level_mult = 1 + (level - 1) * 0.08
    power = species["base_power"] * rarity_mult * element_bonus * level_mult * growth_mult
    defense = species["base_defense"] * rarity_mult * element_bonus * level_mult * growth_mult
    speed = species["base_speed"] * rarity_mult * element_bonus * level_mult * growth_mult
...
def roll_species_for_rarity(rarity: str) -> str:
    ...
    # Редкие стихии (молния/лёд) и мифические (свет/тень) выпадают реже
    weights = [gd.ELEMENT_WEIGHTS.get(gd.SPECIES[k]["element"], 10) for k in pool]
